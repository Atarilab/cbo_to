...
class DoubleCartPoleUnconstrained(Task):
    ...
    def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        ...
        bound_violation_cost = 1000 * self._bound_violation(control)

        # return upright_cost + velocity_cost + control_cost + bound_violation_cost
        return bound_violation_cost

    def terminal_cost(self, state: mjx.Data) -> jax.Array:
        """The terminal cost ϕ(x_T)."""
        upright_cost = 1.0 * self._distance_to_upright(state)
        centering_cost = 1.0 * jnp.square(state.qpos[0])
        velocity_cost = 1.0 * jnp.sum(jnp.square(state.qvel))
